...
def test_register_file_validates_uploads_and_inserts_metadata() -> None:
    ...
    assert inserted_row["checksum_sha256"] == hashlib.sha256(b"hi").hexdigest()
    assert inserted_row["extension"] == ".pdf"
    assert result.id == "file-1"
    assert result.storage_path == "company-1/file-1.pdf"
    assert result.checksum_sha256 == hashlib.sha256(b"hi").hexdigest()


def test_register_file_removes_orphaned_object_when_insert_fails() -> None:
    mock_client = MagicMock()
    ...
    with (
        patch("app.services.file_service.get_supabase_admin", return_value=mock_client),
        patch("app.services.file_service.storage_service.upload_object") as mock_upload,
        patch("app.services.file_service.storage_service.delete_object") as mock_delete,
        patch("app.services.file_service.uuid.uuid4", return_value="file-1"),
    ...
    mock_delete.assert_called_once_with(path="company-1/file-1.pdf")
