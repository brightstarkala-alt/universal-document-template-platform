...
def test_find_cached_pdf_returns_row_when_match_exists() -> None:
    client = _FakeSupabaseQuery(_row())

    result = pdf_generation_service._find_cached_pdf(
        client, file_id="file-1", source_template_id="template-1", generator_version="1.0"
    )
...
def test_find_cached_pdf_returns_none_when_no_match() -> None:
...
def test_next_version_returns_one_when_no_existing_rows() -> None:
    ...
    assert pdf_generation_service._next_version(client, file_id="file-1") == 1


def test_next_version_increments_existing_max() -> None:
    client = _FakeSupabaseQuery({"version": 4})

    assert pdf_generation_service._next_version(client, file_id="file-1") == 5
